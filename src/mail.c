...
void            store_mail(long to, long from, char *message_pointer)
{
     ...
     * message left to write to the file, keep writing the new data blocks
     * and rewriting the old data blocks to reflect where the next block is.
     * Yes, this is kind of a hack, but if the block size is big enough it
     * won't matter anyway.  Hopefully, MUD players won't pour their life
     * stories out into the Mud Mail System anyway.
     * 
     * Note that the block_type data field in data blocks is either a number
     * >=0, meaning a link to the next block, or LAST_BLOCK flag (-2) meaning
     * the last block in the current message.  This works much like DOS' FAT. */
   ...
   header (i.e. the text handed to the player) */
{
    ...
    *(tmstr + strlen(tmstr) - 1) = '\0';
    ...
    sprintf(buf, " * * * * Lands of Myst Mail System * * * *\r\n"
            "Date: %s\r\n"
            "  To: %s\r\n"
                                     ...
                                     int cmd, char *arg)
{
    ...
    sprintf(buf, "$n tells you, 'I'll take %d coins for the stamp.'\r\n"
            "$n tells you, 'Write your message, (/s saves /h help).'",
            STAMP_PRICE);
