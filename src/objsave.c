...
struct obj_data *Obj_from_store_to(struct obj_file_elem object, int zone, int *locate)
{
    struct obj_data *obj;
    int             j;

    if (real_object(object.item_number) > -1) {
        obj = read_object(object.item_number, VIRTUAL, zone, 0);
        *locate = (int) object.locate;
        ...
        GET_OBJ_DATA(obj)=object.data;
        GET_OBJ_RENT(obj) = object.weight;
        GET_OBJ_COST(obj) = object.cost;
        GET_OBJ_TIMER(obj) = object.timer;
        GET_OBJ_SPELL(obj)=object.bound_spell;
        GET_OBJ_SPELL_LEVEL(obj)=object.bound_spell_level;
        GET_OBJ_SPELL_TIMER(obj)=object.bound_spell_timer;
        GET_OBJ_DAMAGE(obj)=object.damage;
        GET_OBJ_ORIG(obj)=object.orig_value;
        obj->obj_flags.bitvector = object.bitvector;
        obj->obj_flags.bitvector2 = object.bitvector2;
        obj->obj_flags.bitvector3 = object.bitvector3;
        strcpy(obj->owner_name, object.owner_name);
        obj->name = str_dup(object.name);
        obj->description = str_dup(object.description);
        obj->short_description = str_dup(object.short_description);
...
struct obj_data *Obj_from_store(struct obj_file_elem object, int zone)
{
    int             locate;

    return Obj_from_store_to(object, zone, &locate);
}
...
int             Obj_to_store_from(struct obj_data * obj, FILE * fl, int locate)
{
    ...
    object.data=GET_OBJ_DATA(obj);
    object.cost=GET_OBJ_COST(obj);
    object.weight = GET_OBJ_RENT(obj);
    object.timer = GET_OBJ_TIMER(obj);
    object.bitvector = obj->obj_flags.bitvector;
    object.bitvector2 = obj->obj_flags.bitvector2;
    object.bitvector3 = obj->obj_flags.bitvector3;
    object.bound_spell=GET_OBJ_SPELL(obj);
    object.bound_spell_level=GET_OBJ_SPELL_LEVEL(obj);
    object.bound_spell_timer=GET_OBJ_SPELL_TIMER(obj);
    object.damage=GET_OBJ_DAMAGE(obj);
    object.orig_value=GET_OBJ_ORIG(obj);

    if (obj->name)
        strncpy(object.name,obj->name ,100);
    if (obj->description)
        strncpy(object.description,obj->description,100);
    if (obj->short_description)
        strncpy(object.short_description, obj->short_description,100);

    for (j = 0; j < MAX_OBJ_AFFECT; j++)
        object.affected[j] = obj->affected[j];
    strcpy(object.owner_name, obj->owner_name);
...
int             Crash_clean_file(char *name)
{
    ...
    if (!get_filename(name, fname, CRASH_FILE))
        return 0;
    /* open for write so that permission problems will be flagged now, at
     * boot time. */
...
void            Crash_listrent(struct char_data * ch, char *name)
{
    ...
    while (!feof(fl)) {
        ...
        if (!feof(fl))
            if (real_object(object.item_number) > -1) {
                obj = read_object(object.item_number, VIRTUAL, 0, 0);
...
void            auto_equip(struct char_data * ch, struct obj_data * obj, int locate)
{
    ...
    if (locate > 0) {           /* was worn */
        ...
        case WEAR_DUALWIELD:
            ...
            break;
        case WEAR_HOLD:
            if (!CAN_WEAR(obj, ITEM_WEAR_HOLD) && CAN_WEAR(obj, ITEM_WEAR_WIELD) && GET_OBJ_TYPE(obj) == ITEM_WEAPON)
                locate = 0;
            break;
        case WEAR_BACKPACK:
...
*/
{
    ...
    if (!free_rent && (rent.rentcode == RENT_RENTED || rent.rentcode == RENT_TIMEDOUT)) {
        ...
        if (cost > GET_GOLD(ch) + GET_BANK_GOLD(ch)) {
            ...
            if (PLR_FLAGGED(ch, PLR_JUSTREIN)) {
                REMOVE_BIT(PLR_FLAGS(ch), PLR_JUSTREIN);
                fclose(fl);
    ...
    while (!feof(fl)) {
        ...
        if (!feof(fl))
            if ((obj = Obj_from_store_to(object, 0, &locate))) {
                auto_equip(ch, obj, locate);

                /* what to do with a new loaded item:
                 * 
                 * if there's a list with <locate> less than 1 below this:
                 * (equipped items are assumed to have <locate>==0 here) then
                 * its container has disappeared from the file gasp* -> put
                 * all the list back to ch's inventory
                 * 
                 * if there's a list of contents with <locate> 1 below this:
                 * check if it's a container - if so: get it from ch, fill
                 * it, and give it back to ch (this way the container has its
                 * correct weight before modifying ch) - if not: the
                 * container is missing -> put all the list to ch's inventory
                 * 
                 * for items with negative <locate>: if there's already a list
                 * of contents with the same <locate> put obj to it if not,
                 * start a new list
                 * 
                 * Confused? Well maybe you can think of some better text to be
                 * put here ...
                 * 
                 * since <locate> for contents is < 0 the list indices are
                 * switched to non-negative */

                if (locate > 0) {       /* item equipped */
                    for (j = MAX_BAG_ROW - 1; j > 0; j--)
                        if (cont_row[j]) {      /* no container -> back to
                                                     * ch's inventory */
                    ...
                    if (cont_row[0]) {  /* content list existing */
                        if (GET_OBJ_TYPE(obj) == ITEM_CONTAINER) {
                            ...
                            obj->contains = NULL;       /* should be empty - but
                                                         * who knows */
                        ...
                        } else {/* object isn't container -> empty content
                                     * list */
                            for (; cont_row[0]; cont_row[0] = obj1) {
                                obj1 = cont_row[0]->next_content;
                    ...
                    if (locate < 0 && locate >= -MAX_BAG_ROW) {
                        /* let obj be part of content list but put it at the
                         * list's end thus having the items in the same order
                         * as before renting */
                        obj_from_char(obj);
    ...
    for (j = 0; j < NUM_WEARS; j++) {
        if (GET_EQ(ch, j)) {
            if (Crash_is_unrentable(GET_EQ(ch, j)))
                /*(IS_OBJ_STAT(GET_EQ(ch, j), ITEM_NORENT) && GET_OBJ_RENT(obj)>=0)) ||
...
void            Crash_idlesave(struct char_data * ch)
{
    ...
    if (GET_LEVEL(ch) < LVL_IMMORT) {
        ...
        if (cost + cost_eq > GET_GOLD(ch) + GET_BANK_GOLD(ch)) {
            for (j = 0; j < NUM_WEARS; j++)     /* unequip player with low
                                                     * money */
                if (GET_EQ(ch, j))
                    obj_to_char(unequip_char(ch, j), ch);
            cost += cost_eq;
                                 ...
                                 int display, int factor)
{
    ...
    if (display) {
        /*    sprintf(buf, "$n tells you, 'Plus, my %d coin fee..'",
            ...
            act(buf, FALSE, receptionist, 0, ch, TO_VICT); */
        sprintf(buf, "$n tells you, 'Rent costs %ld coins%s.'",
                                 ...
                                 int cmd, char *arg, int mode)
{
    ...
    if (CMD_IS("rent")) {
        if (!(cost = Crash_offer_rent(ch, recep, FALSE, mode)))
            return TRUE;
        if (mode == RENT_FACTOR)
            sprintf(buf, "$n tells you, 'Rent will cost you %d coins per day.'", cost);
        else if (mode == CRYO_FACTOR)
            sprintf(buf, "$n tells you, 'It will cost you %d coins to be frozen.'", cost);
        act(buf, FALSE, recep, 0, ch, TO_VICT);
        if (cost > GET_GOLD(ch)) {
            act("$n tells you, '...which I see you can't afford.'",
                FALSE, recep, 0, ch, TO_VICT);
...
void            Crash_save_all(void)
{
    ...
    for (d = descriptor_list; d; d = d->next) {
        if ((d->connected == CON_PLAYING) && !IS_NPC(d->character)) {
            if (PLR_FLAGGED(d->character, PLR_CRASH) && (PRF_FLAGGED(d->character, PRF_AUTOSAVE) || global_save)) {
                if (world[d->character->in_room].number>1)
