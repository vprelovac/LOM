...
char *customer_string(int shop_nr, int detailed)
{
  int sindex, cnt = 1, nlen;
  size_t len = 0;
  static char buf[256];

  for (sindex = 0; *trade_letters[sindex] != '\n' && len + 1 < sizeof(buf); sindex++, cnt <<= 1)
    if (SHOP_TRADE_WITH(shop_nr) & cnt) {
      if (!detailed) {
        strlcpy(buf + len, "_", sizeof(buf) - len);
        len++;
        if (len >= sizeof(buf))
          break;
      }
      ...
      if (detailed) {
	nlen = snprintf(buf + len, sizeof(buf) - len, "%s%s", len > 0 ? ", " : "", trade_letters[sindex]);
        if (len + nlen >= sizeof(buf) || nlen < 0)
          break;
        len += nlen;
      } else if (len + 2 <= sizeof(buf)) {
        buf[len++] = *trade_letters[sindex];
        buf[len++] = '\0';
      } else
        break;
    }
...
void list_all_shops(struct char_data *ch)
{
  ...
  *buf = '\0';
  for (shop_nr = 0; shop_nr <= top_shop && len < sizeof(buf); shop_nr++) {
    ...
    snprintf(buf + len, sizeof(buf) - len, "%3d   %6d   %6d    %s   %3.2f   %3.2f    %s\r\n",
...
void list_detailed_shop(struct char_data *ch, int shop_nr)
{
  ...
  send_to_char(ch, "Buy at:     [%4.2f], Sell at: [%4.2f], Open: [%d-%d, %d-%d]\r\n",
