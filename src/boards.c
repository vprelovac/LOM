...
SPECIAL(gen_board)
{
    ...
    CMD_RESPOND = find_command("respond");

    if (cmd != CMD_WRITE && cmd != CMD_LOOK && cmd != CMD_EXAMINE &&
            cmd != CMD_READ && cmd != CMD_REMOVE && cmd != CMD_RESPOND)
        return 0;
...
void            Board_write_message(int board_type, struct char_data * ch, char *arg)
{
    ...
    *(tmstr + strlen(tmstr) - 1) = '\0';
    ...
    send_to_char("Write your message.  (/s saves /h for help)\r\r\n\n", ch);
...
int             Board_remove_msg(int board_type, struct char_data * ch, char *arg)
{
    ...
    if (msg_storage[slot_num])
        DISPOSE(msg_storage[slot_num]);
    msg_storage[slot_num] = 0;
    msg_storage_taken[slot_num] = 0;
    if (MSG_HEADING(board_type, ind))
        DISPOSE(MSG_HEADING(board_type, ind));
...
void            Board_load_board(int board_type)
{
    ...
    if (!(fl = fopen(FILENAME(board_type), "rb"))) {
        sprintf(bbuf, "Error reading board '%s'", FILENAME(board_type));
        perror(bbuf);
        return;
    }
...
void            Board_reset_board(int board_type)
{
    ...
    for (i = 0; i < MAX_BOARD_MESSAGES; i++) {
        if (MSG_HEADING(board_type, i))
            DISPOSE(MSG_HEADING(board_type, i));
        if (msg_storage[MSG_SLOTNUM(board_type, i)])
            DISPOSE(msg_storage[MSG_SLOTNUM(board_type, i)]);
