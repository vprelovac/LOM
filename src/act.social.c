...
ACMD(do_gemote)
{
...

    {
        ...
        if (!*arg) {
            ...
            sprintf(buf, "&c%s&0", action->others_no_arg);
            ...
            sprintf(buf, "&c%s&0", action->others_auto);
        } else {
            if (GET_POS(vict) < action->min_victim_position)
                act("$N is not in a proper position for that.",
                    FALSE, ch, 0, vict, TO_CHAR | TO_SLEEP);
            else {
                sprintf(buf, "&c%s&0", action->others_found);
            }
        }
    ...
    {       	// its an emote
        ...
        sprintf(buf, "&y[GOSSIP]&c $n %s&0", argument);
        act(buf, FALSE, ch,  0, 0, TO_GMOTE);
