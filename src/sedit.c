...
void            sedit_save_to_disk(struct descriptor_data * d, int new_format);
...
{
    if (S_NOITEM1(shop)) {
        DISPOSE(S_NOITEM1(shop));
        S_NOITEM1(shop) = NULL;
    }
    if (S_NOITEM2(shop)) {
        DISPOSE(S_NOITEM2(shop));
        S_NOITEM2(shop) = NULL;
    }
    if (S_NOCASH1(shop)) {
        DISPOSE(S_NOCASH1(shop));
        S_NOCASH1(shop) = NULL;
    }
    if (S_NOCASH2(shop)) {
        DISPOSE(S_NOCASH2(shop));
        S_NOCASH2(shop) = NULL;
    }
    if (S_NOBUY(shop)) {
        DISPOSE(S_NOBUY(shop));
        S_NOBUY(shop) = NULL;
    }
    if (S_BUY(shop)) {
        DISPOSE(S_BUY(shop));
...
void            free_shop(struct shop_data * shop)
{
    ...
    DISPOSE(S_ROOMS(shop));
    DISPOSE(S_PRODUCTS(shop));
    DISPOSE(shop);
}
    ...
    if (rshop > -1) {           /* The shop already exists, just update it. */
        ...
        for (rshop = 0; rshop < top_shop; rshop++) {
            if (!found) {       /* Is this the place? */
                if (SHOP_NUM(rshop) > OLC_NUM(d)) {     /* Yep, stick it in
                                                                 * here. */
                    found = 1;
...
void            sedit_save_to_disk(struct descriptor_data * d, int new_format)
{
    ...
    for (i = zone * 100; i <= top; i++) {
        if ((rshop = real_shop(i)) != -1) {
            ...
            /* Save messages'n'stuff. Added some small'n'silly defaults as
             * sanity checks. */
            fprintf(shop_file,
                    "%s~\n%s~\n%s~\n%s~\n%s~\n%s~\n%s~\n"
                    "%d\n%d\n%d\n%d\n",
    ...
    {
        sprintf(buf2, "%s",  zone_table[OLC_ZNUM(d)].disk_name);
        /* We're fubar'd if we crash between the two lines below. */
        remove(buf2);
        rename(fname, buf2);
    }
