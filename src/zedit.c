...
void            zedit_save_to_disk(struct descriptor_data * d, int new_format);
...
void            zedit_setup(struct descriptor_data * d, int room_num)
{
    ...
    strcpy(zone->reset_msg, zone_table[OLC_ZNUM(d)].reset_msg);
...
void            zedit_new_zone(struct char_data * ch, int vzone_num)
{
     ...
     * crashes reported to the CircleMUD list. It was happily overwriting the
     * stack.  This new loop by Andrew Helm fixes that problem and is more
    ...
    *(new_table[i].disk_name)=0;
    strcpy(new_table[i].reset_msg,"You hear some squeaking sounds...");
    new_table[i].creator = str_dup(GET_NAME(ch));
    ...
    /* Previously, creating a new zone while invisible gave you away. That
     * quirk has been fixed with the MAX() statement. */
...
void            zedit_save_internally(struct descriptor_data * d)
{
    ...
    /* Delete all entries in zone_table that relate to this room so we can
     * add all the ones we have in their place. */
    while (ZCMD.command != 'S') {
        switch (ZCMD.command) {
        case 'M':
        case 'O':
    ...
    if (OLC_ZONE(d)->number) {
        DISPOSE(zone_table[OLC_ZNUM(d)].name);
        ...
        strcpy(zone_table[OLC_ZNUM(d)].reset_msg,OLC_ZONE(d)->reset_msg);
    }
...
void            zedit_save_to_disk(struct descriptor_data * d, int new_format)
{
    ...
    if (!new_format)
        sprintf(fname, "%s/%d.new", ZON_PREFIX, zone_table[OLC_ZNUM(d)].number);
    else
    ...
    fprintf(zfile, "#%d\n" "%s~\n" "%d %d %d\n\n",
            zone_table[OLC_ZNUM(d)].number,
            (zone_table[OLC_ZNUM(d)].name && *zone_table[OLC_ZNUM(d)].name)
            ? zone_table[OLC_ZNUM(d)].name : "undefined",
            zone_table[OLC_ZNUM(d)].top,
            zone_table[OLC_ZNUM(d)].lifespan,
            zone_table[OLC_ZNUM(d)].reset_mode
           );
    if (zone_table[OLC_ZNUM(d)].creator)
        fprintf(zfile, "#AUTHOR %s~\n\n",         zone_table[OLC_ZNUM(d)].creator);


    fprintf(zfile, "#RESETMSG %s~\n\n",         zone_table[OLC_ZNUM(d)].reset_msg);
    ...
    fclose(zfile);
    if (!new_format)
    {     sprintf(buf2, "%s/%d.zon", ZON_PREFIX, zone_table[OLC_ZNUM(d)].number);
        // else
        //	sprintf(buf2, "%s.new",  zone_table[OLC_ZNUM(d)].disk_name);
...
void            remove_cmd_from_list(struct reset_com ** list, int pos)
{
    ...
    /* Value is 'count' because we didn't include the terminator above but
     * since we're deleting one thing anyway we want one less. */
...
void            zedit_disp_menu(struct descriptor_data * d)
{
    ...
    sprintf(buf,
            ...
            grn, nrm, yel, OLC_ZONE(d)->creator ? OLC_ZONE(d)->creator : "<NONE!>",
            grn, nrm, yel, OLC_ZONE(d)->reset_msg ? OLC_ZONE(d)->reset_msg : "<NONE!>",
...
void            zedit_parse(struct descriptor_data * d, char *arg)
{
    ...
    case ZEDIT_MAIN_MENU:
        ...
        case 'A':
            /* Edit zone name. */
            send_to_char("Enter new author name : ", d->character);
            OLC_MODE(d) = ZEDIT_ZONE_AUTHOR;
            break;
        case 'm':
        case 'M':
            /* Edit zone name. */
            send_to_char("Enter new reset message : ", d->character);
            OLC_MODE(d) = ZEDIT_ZONE_MSG;
            break;
        case 't':
        case 'T':
            /* Edit top of zone. */
    ...
    case ZEDIT_COMMAND_TYPE:
        ...
        if (!OLC_CMD(d).command ||
                (!strchr("MOPEDGR", OLC_CMD(d).command))) {
    ...
    case ZEDIT_ARG3:
        ...
        case 'E':
            pos = atoi(arg);
            /* Count number of wear positions.  We could use NUM_WEARS, this
             * is more reliable. */
            while (*equipment_types[i] != '\n')
                i++;
            if ((pos < 0) || (pos > i))
    ...
    case ZEDIT_ZONE_AUTHOR:
        /* Add new author and return to main menu. */
        if (OLC_ZONE(d)->creator)
            DISPOSE(OLC_ZONE(d)->creator);
        else
            log("SYSERR: OLC: ZEDIT_ZONE_AUTHOR: no name to free!");
    ...
    case ZEDIT_ZONE_MSG:
        strncpy(OLC_ZONE(d)->reset_msg,arg, 99);
        OLC_ZONE(d)->number = 1;
        zedit_disp_menu(d);
        break;
