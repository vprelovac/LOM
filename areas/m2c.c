                   ...
                   int *d1, int *d2, int *d3, int *gold, int *exp)
{
  ...
  switch (mobconv) {
    ...
    case MIDVALS:
      ...
      *exp = get_exp(level); 
      ...
      break;
   default:
	if ((*thac0<0) || (*thac0>20))   *thac0 = interpolate (level,20,0);   
	if ((*ac<-15) || (*ac>15))   *ac = interpolate (level,10,-10);   
 	break;     
  }
  if (dam > 0) {
    *d3 = dam / 3;
 ...
 if (level>50 || level<1) printf("W A R N I N G: Mob %d has level set to %d\n",num,level);
// if (((*h1)*(*h2)+(*h3))==0) printf("Warning: Mob %d has 0 hitpoints\n",num);
 if (((*h1)*((*h2+1)/2)+(*h3))>500) printf("Warning: Mob %d has %d (%d) hitpoints\n",num,((*h1)*((*h2+1)/2)+(*h3)),((*h1)*(*h2)+(*h3)));
// if (((*d1)*(*d2)+(*d3))==0) printf("Warning: Mob %d does no damage\n",num); 
 if (((*d1)*((*d2+1)/2)+(*d3))>40) printf("Warning: Mob %d does %d (%d) damage\n",num,(*d1)*((*d2+1)/2)+(*d3),(*d1)*(*d2)+(*d3)); 
 if (*gold==0) printf("Warning: Mob %d has no gold\n",num); 
 if (*exp==0) printf("Warning: Mob %d has no exp\n",num);  
...
int check_spell (int splnr, int objnr)
{
  ...
  if (splnr <= 44) {
    ...
    switch (splnr) {
     ...
     default:
        if (warn)
          printf("Unknown spellnr %d in obj %d. Set to UNKNOWN\n",splnr,objnr);
        nsplnr = 0;
        found = TRUE;
        break;
    }
  ...
  if (!found) {
    if (warn)
      printf("Unsupported SPELL found in obj %d. Set to UNKNOWN.\n",objnr);
    nsplnr = 0;
  }
  ...
  for (i = 0; i <= 31;i++) {
    switch (i) {
      ...
      case 20:
        if (IS_SET(mflg,(1 << i))) {
          if (warn)
            printf("Unsupported Mob Affection - %s at mob %d. Removing\n",get_maffs(i),mobnr);
        }
        break;
      case 9:
        if (IS_SET(mflg, (1 << 9))) {
      ...
      default:
        if (IS_SET(mflg,(1 << i))) {
          if (warn)
            printf("Unknown MOB AFF in mob %d. It has been removed.\n",mobnr);
        }
        break;
    }
  }
  ...
  for (i = 0; i <= 31;i++) {
    switch (i) {
      ...
      case 10:
        if (IS_SET(mflg,(1 << i))) {
          if (warn)
            printf("Unsupported Mob Flag - %s at mob %d. Removing\n",get_mflag(i),mobnr);
        }
        break;
      default:
        if (IS_SET(mflg,(1 << i))) {
          if (warn)
            printf("Unknown MOB FLAG in mob %d. It has been removed.\n",mobnr);
        }
        break;
    }
  }
  ...
  for (i = 0; i <= 31;i++) {
    switch (i) {
      ...
      case 12:
        if (IS_SET(oflg,(1 << i))) {
          if (warn)
            printf("Unsupported Obj Flag - %s at obj %d. Removing\n",get_oflag(i),objnr);
        }
        break;
      case 13:
        if (IS_SET(oflg,(1 << i))) {
          if (warn)
            printf("Obj %d has NOREMOVE flag set\n",objnr);
             SET_BIT(noflg,(1 << i));
        }
        break;
      default:
  ...
  if (tpe >= 23) {
    if (tpe == 25) {
      tpe = 23;
        ...
        tpe = 10;
        if (warn)
          printf("ITEM PILL found at obj %d. Setting to Potion.\n",objnr);
    } else if (tpe == 23 || tpe == 24) {
      if (warn)
        printf("ITEM CORPSE found at obj %d. Setting to container.\n",objnr);
      tpe = 15;
    } else {
      if (warn)
        printf("Unknown ITEM Type found at obj %d. Setting to Other.\n",objnr);
      tpe = 12;
    }
  }
  return (tpe);
  ...
  for (i = 0; i <= 31; i++) {
    switch (i) {
      ...
      case 12:
        if (IS_SET(rflg,(1 << 12))) {
          if (warn)
            printf("PET SHOP flag on room %d. Add to spec_assign.c.\n",maxroom);
        }
        break;
      case 14:
        if (IS_SET(rflg,(1 << 14))) {
          if (warn) 
            printf("Nonstandard MERC ROOM FLAG at room %d. silence/bank who knows?\n",maxroom);
        }
        break;
      default:
        if (IS_SET(rflg,(1 << i))) {
          if (warn)
            printf("Unknown ROOM FLAG in room %d. It has been removed.\n",maxroom);
        }
        break;
    }
  }
...
int check_sectors (int stpe)
{
  ...
  if (stpe > 9) {
    if (stpe == 10) {
      if (warn)
        printf("Sectortype DESERT at room %d set to QUICKSAND.\n",maxroom);
      stpe = 10;
    } else if (stpe == 11) {
      if (warn)
        printf("Nonstandard MERC Sector at room %d. Setting to Field.\n",maxroom);
      stpe = 2;
    } else {
      if (warn)
        printf("Unknown sectortype at room %d. Setting to Field.\n",maxroom);
      stpe = 2;
    }
  }
  ...
  for (;;) {
    ...
    if (*ch != 'S') {
      if (sscanf(ch," %c %d %s ",&tpe,&nr,spec) != 3) {
        printf("Error in #SPECIALS section.\n");
        exit (1);
      } else {
        switch (tpe) {
          case 'M':
  ...
  for (;;) {
    ...
    if (*ch != 'S') {
      if (*ch != 'C' && *ch != 'R' && *ch != 'F') {
        ...
        if (warn)
          printf("Unsupported Reset command : %s\n",ch);
      }
    } else {
     fprintf(fp,"S\n");
     fprintf(fp,"$~\n");
  ...
  for (;;) {
    ...
    if (sscanf(ch,"#%d ",&num) == 1) {
      ...
      if (sscanf (ch,"%s %s %s %s",s10,s20,s30,s40) != 4) {
        printf("Error at obj %d 2nd numeric line: %s\n",num,ch);
        exit (1);
      }
      ...
      if (tpe == 5) {                     /* Weapons */
        if (t4 < 1) {
          if (warn)
            printf("Weapon object %d is type hit. Setting to Bludgeon\n",num);
          t4 = 5;
        }
        if (t2*t3 > 50 && warn)
          printf("Weapon object %d might give excessive damage.\n",num);
        if (t2 < 1 || t3 < 1) {
          if (warn && maxrand < 1)
            printf("Weapon object %d will give no damage.\n",num);
          if (maxrand > 1) {
...
/*        t2 = check_spell (t2,num);
        ...
        if (t2 < 1 && t3 < 1 && t4 < 1 && warn)*/
          printf("Potion/Scroll object %d has needs spell assigned.\n",num);
...
/*        if (t1 < 1 && randspells)
        ...
        if ((t1 < 1 || t2 < 1 || t3 < 1) && !randspells && warn)
          printf("Some values in staff/wand object %d is 0.\n",num);
        t4 = check_spell (t4,num);
        if (t4 < 1 && warn)*/
          printf("Staff/Wand object %d needs spell assigned.\n",num);
      }
      fprintf (fp,"%d %d %d %d\n",t1,t2,t3,t4);
      get_line (fl,ch);
      if (sscanf (ch," %d %d %d ",&t1,&t2,&t3) != 3) {
        printf("Error at obj %d 3rd numeric line.\n",num);
        exit (1);
      }
      fprintf (fp,"%d %d %d\n",t1,t2,t3);
      found = -1;
          ...
          case 'A':
            ...
            if (sscanf(ch,"A %d %d ",&t1,&t2) != 2) {
              ...
              if (sscanf(ch," %d %d ",&t1,&t2) != 2) {
                printf("Error at obj %d in Affections.\n",num);
                exit (1);
              }
            }
            if (numaffs <= MAX_OBJ_AFFS) {
if (t1==18 && t2>10) printf("Object %d has hitroll of %d\n",num,t2);
if (t1==19 && t2>10) printf("Object %d has damroll of %d\n",num,t2);
if (t1==7 || t1==8 || t1==15 || t1==16) printf("Object %d has touched forbidden (%d)\n",num,t2);
else{              fprintf(fp,"A\n");
              fprintf(fp,"%d %d\n",t1,t2);}
            } else {
             if (warn)
               printf("Object %d has to many AFFECTIONS!!!.\n",num);
            }
            break;
          case 'E':
            fprintf(fp,"E\n");
      ...
      while (found == -1) {
        ...
        switch (*ch) {
          ...
          case 'D':
            ...
            get_line (fl,ch);
            if (sscanf(ch," %d %d %d ",&t1,&t2,&t3) != 3) {
              printf("At room %d error in Directional Data.\n",num);
              exit (1);
            }
            if (nznr >= 0 && t3 > 0)
              t3 = change_number(t3);
...
void count_rooms (FILE *fl) {
  ...
  if (feof(fl)) {
    printf("Could not find the #ROOMS section.\n");
    exit(1);
  }
  ...
  for (;;) {
    ...
    if (sscanf(ch,"#%d ",&num) == 1) {
      ...
      if (sscanf (ch," %d %c ",&algn,&mtpe) != 2) {
        printf("Error at mob %d 1st numeric line.\n",num);
        exit (1);
      }
if (!cifre) fprintf(fp,"%s %s %d %c\n",get_ascii(flgs),get_ascii(affs),algn,mtpe);
      ...
      if (sscanf (ch," %d %d %d %dd%d+%d %dd%d+%d",&t1,&t2,&t3,&t4,&t5,&t6,&t7,&t8,&t9) != 9) {
        printf("Error at mob %d 2nd numeric line.\n",num);
        exit (1);
      }
      get_line (fl,ch);
      if (sscanf (ch," %d %d ",&mgold,&mexp) != 2) {
        printf("Error at mob %d 3rd numeric line.\n",num);
        exit (1);
      }
      ...
      if (sscanf (ch," %d %s %d ",&t1,s1,&t3) != 3) {
        printf("Error at mob %d 4th numeric line.\n",num);
        exit (1);
      }
      if (t1==0 || t1==7) t1=8;
      fprintf (fp,"%d %d %d\n",t1,t1,t3);
...
int main (int argc, char *argv[])
{
    ...
    if (*(argv[arg]) == '-') {
      switch (*(argv[arg]+1)) {
        ...
        case 'r' :
          ...
          break;
        case 'h':
          printf("Usage: merc2circle [options] mercfile.are\n");
  ...
  while (!feof(fl)) {
    ...
    if (!feof(fl))
      ...
      if (!strcmp("#ROOMS",ch)) {
        parse_rooms(fl);
        printf("Finished Converting Rooms.\n\n");
      }
      if (!strcmp("#OBJECTS",ch)) {
        parse_objects(fl);
        printf("Finished Converting Objects.\n\n");
      }
      if (!strcmp("#MOBILES",ch)) {
        parse_mobiles(fl);
        printf("Finished Converting Mobs.\n\n");
      }
      if (!strcmp("#MOBPROGS",ch)) {
        printf("MOBPROGS section exists!\n\n");
      }
      
      if (!strcmp("#RESETS",ch)) {
        parse_resets(fl);
        printf("Finished Converting Resets.\n\n");
      }
      if (!strcmp("#SHOPS",ch)) {
        parse_shops(fl);
        printf("Finished Converting Shops.\n\n");
      }
      if (!strcmp("#SPECIALS",ch)) {
        parse_specials(fl);
        printf("Finished Converting Specials.\n\n");
      }
      if (!strcmp("#HELPS",ch)) {
        parse_helps(fl);
        printf("Finished Copying Help.\n\n");
      }
  }
